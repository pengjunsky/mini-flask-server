...
class Order(Base):
    ...
    def create_order(product_ids, address_id, user_coupon_id, remark, uid):
        ...
        with db.auto_commit():
            ...
            order.remark = remark
            order.total_price = Order.product_to_calculate(o_products)[0]
            order.postage = Order.product_to_calculate(o_products)[1]
            OrderSnap.add_order_snap(o_products, order_no)
            Order.minus_product_stock(o_products)
    ...
    @staticmethod
    def product_to_calculate(o_products):
        total_price = 0
        multiple = 100
        nums = []
        for i in o_products:
        ...
        postage = heapq.nsmallest(1, nums)
        return total_price, postage

    @staticmethod
    def minus_product_stock(o_products):
        for i in o_products:
            with db.auto_commit():
                if i['property']:
